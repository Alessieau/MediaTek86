...
**Titre :** MediaTek86 (Gestion du personnel)
...
L'application respecte l'architecture Modèle Vue Contrôleur permettant séparer l'affichage, la logique métier et l'accès aux données.

4 paquetages principaux composent le code :



**Vue** : Contient les interfaces graphiques (les fenêtres FrmMain, FrmAbsences, FrmLogin).



**Contrôleur** : Fait le pont entre les vues et les données. Il contient la logique métier de l'application.
...
**DAL** (Data Access Layer) : Gère la connexion à la base de données MySQL et l'exécution des requêtes SQL (AccesDonnees)
